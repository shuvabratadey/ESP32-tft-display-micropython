...
class ST77xx:
    ...
    def circle_helper(self, x0, y0, r, cornername, color):
        f = 1 - r
        ddF_x = 1
        ddF_y = -2 * r
        x     = 0
        while (x < r):
            ...
            f += ddF_x
            if (cornername & 0x4):
                self.pixel(x0 + x, y0 + r, color)
                self.pixel(x0 + r, y0 + x, color)
    ...
    def fill_circle_helper(self, x0, y0,  r, cornername, delta, color):
        ...
        while (y < r):
            if (f >= 0):
                if (cornername & 0x1): self.hline(x0 - y, y0 + r, y + y + delta, color)
                if (cornername & 0x2): self.hline(x0 - y, y0 - r, y + y + delta, color)
                r-=1
                ddF_y += 2
                f     += ddF_y
            y+=1
            ...
            if (cornername & 0x1): self.hline(x0 - r, y0 + y, r + r + delta, color)
            if (cornername & 0x2): self.hline(x0 - r, y0 - y, r + r + delta, color)
    ...
    def fill_circle(self, x0, y0, r, color):
        ...
        self.hline(x0 - r, y0, dy+1, color)

        while(xo<r):
            if(p>=0):
                self.hline(x0 - xo, y0 + r, 2 * xo+1, color)
                self.hline(x0 - xo, y0 - r, 2 * xo+1, color)
                dy-=2
                p-=dy
                r-=1
            ...
            self.hline(x0 - r, y0 + x, 2 * r+1, color)
            self.hline(x0 - r, y0 - x, 2 * r+1, color)
            x+=1
            
    def round_rect(self, x, y, w, h, r, color):
        self.hline(x + r  , y    , w - r - r, color)
        self.hline(x + r  , y + h - 1, w - r - r, color)
        self.vline(x    , y + r  , h - r - r, color)
        self.vline(x + w - 1, y + r  , h - r - r, color)
    ...
    def fill_round_rect(self, x, y, w, h, r, color):
        self.fill_rect(x, y + r, w, h - r - r, color)
        self.fill_circle_helper(x + r, y + h - r - 1, r, 1, w - r - r - 1, color)
        self.fill_circle_helper(x + r    , y + r, r, 2, w - r - r - 1, color)
    ...
    def fill_triangle(self, x0, y0, x1, y1, x2, y2, color):
        ...
        if (y0 == y2):
            var_a = var_b = x0
            if (x1 < var_a): var_a = x1
            elif (x1 > var_b): var_b = x1
            if (x2 < var_a):      var_a = x2
            elif (x2 > var_b): var_b = x2
            self.hline(var_a, y0, var_b - var_a + 1, color)
            return
        dx01 = x1 - x0
        dy01 = y1 - y0
        dx02 = x2 - x0
        ...
        for y in range(y0, last+1):
            ...
            sa += dx01
            sb += dx02
            if (var_a > var_b): var_a, var_b = var_b, var_a
            self.hline(int(var_a), int(y), int(var_b - var_a + 1), color)
        sa = dx12 * (y - y1)
        sb = dx02 * (y - y0)
